#!/usr/bin/env python3
"""
Generate lightweight waveform and spectrum plots for the measured clean-signal
matrix used in the README bonus section.
...
while the dominant/adaptive numbers in the summaries come from the measured DUT
sessions already captured in the repo.
"""
...
SIGNALS = [
    {
        ...
        "observed_dominant_hz": 5.00,
        "adaptive_fs_hz": 10.00,
        "session_dir": "tools/plot_sessions/20260422_120509_clean_dut_no_ina219_60s_v2",
        "note": "baseline case where the dominant and highest tone are the same, so adaptation matches the Nyquist expectation cleanly.",
    ...
    },
    {
        ...
        "note": "the 9 Hz tone exists, but the 3 Hz tone is stronger, so the controller follows the dominant peak instead of the highest-frequency component.",
    ...
    },
    {
        ...
        "adaptive_fs_hz": 10.10,
        "session_dir": "tools/plot_sessions/20260424_104628_clean_c_dut",
        "note": "with three tones present, the 5 Hz component still dominates, so the adaptive rate stays close to 10 Hz instead of rising toward 14 Hz.",
